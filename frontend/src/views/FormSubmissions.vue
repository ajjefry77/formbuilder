<template>
  <div class="page">
    <div class="page-header">
      <div>
        <router-link :to="`/forms/${route.params.id}/edit`" class="back-link">← برگشت</router-link>
        <h1 class="page-title" style="margin-top:4px">پاسخ‌های فرم: {{ form?.title }}</h1>
      </div>
    </div>
    ...
    <template v-else>
      <div class="filters card">
        <div class="filter-section">
          <div class="filter-section__title">بازه زمانی و مرتب‌سازی</div>
        ...
        <div class="filter-section">
          ...
          <div v-for="(cond, i) in fieldFilters" :key="i" class="condition-row">
            <label class="condition-field">
              <span>فیلد</span>
              <select v-model="cond.field_key" class="select-native" @change="onFieldChange(i)">
                <option value="">انتخاب فیلد</option>
                <option v-for="field in form?.fields || []" :key="field.id" :value="`field_${field.id}`">
                  {{ field.label }}
                </option>
              </select>
            </label>
            <label class="condition-operator">
              ...
              <select v-model="cond.operator" class="select-native">
                <option v-for="op in getOperatorsForField(i)" :key="op.value" :value="op.value">
                  {{ op.label }}
                </option>
              </select>
            </label>
            <label class="condition-value" v-if="!isNoValueOp(cond.operator)">
              <span>مقدار</span>
              <input v-model="cond.value" class="input" type="text" placeholder="مقدار..." />
            </label>
            <button class="btn btn-danger btn-sm condition-remove" @click="removeCondition(i)" :disabled="fieldFilters.length === 1">✕</button>
          </div>

          <button class="btn btn-ghost btn-sm" @click="addCondition">+ افزودن شرط</button>
        </div>
      ...
      <div v-if="!submissions.length" class="empty-state card">
        ...
        <router-link :to="`/forms/${route.params.id}/preview`" class="btn btn-primary" style="margin-top:12px">
          مشاهده فرم
        </router-link>
      </div>
      <div v-else>
            ...
            <tbody>
              <tr v-for="(sub, idx) in submissions" :key="sub.id">
                ...
                <td v-for="field in form.fields" :key="field.id">
                  <span class="cell-val">{{ formatValue(sub.data[field.id]) }}</span>
                </td>
...
function getFieldType(i) {
  ...
  const field = form.value?.fields?.find((f) => `field_${f.id}` === cond.field_key)
...
async function resetFilters() {
  ...
  fieldFilters.splice(0, fieldFilters.length, { field_key: '', operator: 'contains', value: '' })
  await applyFilters()
}
...
.filter-section__title { font-size: 12px; font-weight: 600; color: var(--text-muted); margin-bottom: 8px; }
.filter-section__row { display: flex; flex-wrap: wrap; gap: 12px; }
.filter-section__row--logic { margin-bottom: 8px; }
...
.logic-toggle { display: flex; align-items: center; gap: 8px; }
.logic-toggle span { font-size: 12px; color: var(--text-muted); white-space: nowrap; }
.logic-toggle .select-native { width: auto; min-width: 100px; }
.condition-row { display: flex; flex-wrap: wrap; gap: 8px; align-items: flex-end; margin-bottom: 8px; }
.condition-field { display: flex; flex-direction: column; gap: 4px; min-width: 160px; flex: 1; }
.condition-field span { font-size: 11px; color: var(--text-muted); }
.condition-operator { display: flex; flex-direction: column; gap: 4px; min-width: 130px; }
.condition-operator span { font-size: 11px; color: var(--text-muted); }
.condition-value { display: flex; flex-direction: column; gap: 4px; min-width: 140px; flex: 1; }
.condition-value span { font-size: 11px; color: var(--text-muted); }
.condition-remove { margin-bottom: 0; height: 36px; width: 36px; padding: 0; display: flex; align-items: center; justify-content: center; }
